package com.github.sftwnd.crayfish.examples.akka.fsm.hackerfsm;
...
public class DiningHakkersOnFsm {
  ...
  */
  public static class Chopstick extends AbstractLoggingFSM<CS, TakenBy> {
    ...
    when(CS.Available,
      matchEventEquals(Messages.Take, (take, data) ->
        goTo(CS.Taken).using(new TakenBy(getSender())).replying(new Messages.Taken(self()))));
    ...
    when(CS.Taken,
      matchEventEquals(Messages.Take, (take, data) ->
              ((AbstractFSM<CS, TakenBy>)this).stay().replying(new Messages.Busy(self()))).
      event((event, data) -> (event == Messages.Put) && (data.hakker == getSender()), (event, data) ->
        goTo(CS.Available).using(new TakenBy(context().system().deadLetters()))));

    // Initialize the chopstick
      ((AbstractFSM<CS, TakenBy>)this).initialize();
    }
  }
  ...
  */
  public static class Hakker extends AbstractLoggingFSM<HS, TakenChopsticks> {
    ...
    when(HS.Waiting,
      matchEventEquals(Messages.Think, (think, data) -> {
    ...
    when(HS.Thinking,
      matchEventEquals(StateTimeout(), (event, data) -> {
        left.tell(Messages.Take, self());
        right.tell(Messages.Take, self());
    ...
    when(HS.Hungry,
      matchEvent(Messages.Taken.class, (taken, data) -> taken.chopstick == left,
        (taken, data) -> goTo(HS.WaitForOtherChopstick).using(new TakenChopsticks(left, null))).
      event(Messages.Taken.class, (taken, data) -> taken.chopstick == right,
    ...
    when(HS.WaitForOtherChopstick,
      ...
      event(Messages.Busy.class, (busy, data) -> {
        if (data.left != null) left.tell(Messages.Put, self());
        if (data.right != null) right.tell(Messages.Put, self());
    ...
    when(HS.FirstChopstickDenied,
      matchEvent(Messages.Taken.class, (taken, data) -> {
        taken.chopstick.tell(Messages.Put, self());
        return startThinking(Duration.create(10, MILLISECONDS));
      }).
      event(Messages.Busy.class, (busy, data) ->
  ...
  */
  public static void main(String[] args) {
    ...
    hakkers.stream().forEach(hakker -> hakker.tell(Messages.Think, ActorRef.noSender()));
  }
}
